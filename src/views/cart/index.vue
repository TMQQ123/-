        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
              ...
              <td class="tc">
                <XtxNumbox @change="($event)=>undateCount(goods.skuId,$event)" :max="goods.stock " :modelValue="goods.count" />
              </td>
              <td class="tc">
...
export default {
  ...
  setup() {
    ...
    // 批量删除选中的商品 也支持鼻梁清空无效商品
    const batchDeleteCart = (isclear) => {
      Confirm({
        text: `亲，您是否确认删除${isclear ? "失效的" : "选中的"}商品`,
      })
        .then(() => {
    ...
    const checkout = () => {
      // 1. 判断是否选中商品且提示
      // 2. 弹出确认框 提示：下单结算需要登录
      // 3. 使用导航守卫 遇见需要登录的路由跳转 拦截到登录
      if (store.getters["cart/selectedList"].length === 0)
        return Message({ text: "至少勾选一件商品结算" });
      // 如果登录直接跳转
      if (store.state.user.profile.token) {
        return router.push("/member/checkout");
      }
      // 未登录弹出确认框
      Confirm({ text: "下单结算需要登录，确定现在去登陆吗？" })
        .then(() => {
